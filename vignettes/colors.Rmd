...
The following examples illustrate how plotting functions in R can use the **unikn** color palettes, as well as the `seecol()`, `usecol()`, `newpal()`, and `grepal()` functions. 

### Plotting in **base** R 

**Task:** Using **unikn** colors in a visualization created by **base** R graphics. 

When using the `plot()` function of **base R** for a scatterplot, we can 
...
**Task:** Using **unikn** colors in a visualization created by the **ggplot2** package.

When using the `ggplot()` function of **ggplot2** (e.g., for an area plot), we can use the `usecol()` function for defining a color palette that is used as the `values` of the 
`scale_color_manual()` or `scale_fill_manual()` functions:
...
```{r newpal-example, fig.width = 6, fig.asp=.9, fig.align = 'center'}
