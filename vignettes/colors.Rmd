...
2. `grepal()` allows **searching for color names** (i.e., a vector of color names or a palette) for a `pattern` and returns those colors that match the pattern (or regular expression);  

3. `newpal()` allows **defining new color palettes** (as vectors or data frames with dedicated color names).  
...
2. `shades_of()` provides a color gradient or `n`\ **shades of some color**. 


<!-- Transition: -->

The rest of this vignette provides examples of using and some details on these functions. 
