...
A particular color palette (e.g., `pal_seegruen`) can not only be displayed by providing it (as an object) but also by providing its name (i.e., `"pal_seegruen"`) or even an incomplete object name or name (i.e., `"seegruen"` or `seegruen`). Hence, the following all yield the same result: 

<!-- +++ here now +++: Still true in 2020? -->
...
seecol_2(list(my_pair, pal1, pal2, pal_unikn, pal_unikn_pair), 
...
# Scaled version: 
seecol_2(list(pal_freiburg_basic, pal_princeton_1, pal_mpg, pal_unikn), n = 9,
