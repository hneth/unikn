...
Based on the setting of `pal`, the `seecol()` function distinguishes between two modes:
  ...
  - _comparing_ a set of multiple color palettes (when providing a keyword or `list`-object);  
  - viewing the _details_ of a single color palette (when providing only one color palette).   
...
Three additional functions provide general color functionality:

1. `newpal()` allows **defining new color palettes** (usually as data frames with dedicated color names); 

2. `simcol()` allows **finding similar colors** (given a target color, a set of candidate colors, and some tolerance threshold(s)); 

3. `grepal()` allows **searching the names of color palettes** (i.e., a vector of color names or data frame of named colors) for a `pattern` and returns those colors that match the pattern (or regular expression). 


<!-- C. Auxiliary functions: -->

Finally, the follwing functions serve in auxiliary roles: 
