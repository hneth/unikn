...
2. by RGB values: 
Triple of numeric values on 3 dimensions (red, green, blue). 
Range of values is either from\ 0 to\ 1 or from\ 0 to\ 255. 

3. by HEX values: 
Hexadecimal character strings (ranging from\ "#000000" to\ "#FFFFFF") correspond to decimal RGB values (in the 0 to 255 range).

<!-- Note:  -->

When viewing an existing color palette with the `seecol()` function of **unikn**, the name, HEX and RGB values of the individual colors are being shown:^[Both RGB and HEX values also allow for a fourth dimension (usually named `alpha`) that denotes color transparency (or opacity). 
...
Representing colors (RGB and HEX values):

```{r rgb-col-def, eval = FALSE}
# A: Maximize contrast (on all 3 dimensions):
...
# Note intermediate cases:
seecol(usecol(c(c_0, c_1), n = 256), 
              # col_brd = "grey33", lwd_brd = .50,
              main = "All 256 shades of grey")
