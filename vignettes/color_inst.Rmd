...
Thus, when aiming to create a new color palette, we need to 

1. define its elementary color objects, and then 

2. combine them into an R data structure (usually a data frame or named vector). 

In the following, we illustrate both steps in an example. 
...
```{r def-col-mpg-example, fig.width = 5, fig.asp = .80, fig.align = 'center'}
...
Unfortunately, some definitions (such as those using the subtractive CMYK model) are mostly geared towards printing (as opposed to computer screens) and other standards (such as numeric PANTONE&trade; PMS codes) are proprietary. 
...
The decimal notation system uses different symbols for 10 distinct numeric values from\ $0$ to\ $9$ and a place-value notation to express numeric values beyond\ $9$ as polynomials. For instance, the three digit notation\ $123$ is actually an abbreviation for the numeric value $1 \cdot 10^2 + 2 \cdot 10^1 + 3 \cdot 10^0$. 

A hexadecimal notation system extends this convention by using different symbols for 16 distinct numeric values from\ $0$ to\ $15$. 
The decimal values\ $10$ to\ $15$ are represented by the letters\ $A$ to\ $F$.
