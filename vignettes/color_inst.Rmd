...
The representation of color is a wide-ranging topic that has fascinated scholars and practicioners for centuries. 
Essentially, there are many alternative ways to analyze or synthesize colors, and the most appropriate representational system depends on the specific context of a particular use case (see [Wikipedia: Color model](https://en.wikipedia.org/wiki/Color_model) for an introduction). 

In R, the **grDevices** package (included in any default installation) provides colors and color-related functions. 
Although R supports multiple color models, the three most common ways of expressing colors in R are:
...
3. by HEX codes: 
character strings "#000000" to "#FFFFFF" correspond to RGB values (in the 0 to 255 range).

Note: 
Both 2. and 3. also allow for a fourth dimension "alpha" that denotes color opacity / transparency. 
