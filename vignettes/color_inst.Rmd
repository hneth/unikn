...
We will first use the RGB values to define both colors (as these are provided in the definition), but then show that we could have used their HEX values (if they had been provided) to define the same color objects.
...
Whereas the [sRGB standard](https://en.wikipedia.org/wiki/SRGB) expresses all values in a normalized range of\ $[0, 1]$, most color definitions use the traditional 8-bit representation that distinguish between $2^8 = 256$\ levels in the range of\ $[0, 255]$. 
...
When evaluating an object created by `rgb()`, R returns a character string that expresses the same three values (for the `red`, `green`, and `blue` dimension, respectively) in hexadecimal (HEX) notation, where the ten digits\ 0 to\ 9 are extended by six letters\ A to\ F to turn our typical decimal notation into a place-value notation with a base of\ 16. 
...
```{r def-col-rgb-50pc}
mpg_green_50 <- grDevices::rgb( 17, 102,  86, alpha = 256/2, maxColorValue = 255)
mpg_grey_50  <- grDevices::rgb(221, 222, 214, alpha = 256/2, maxColorValue = 255)
```
