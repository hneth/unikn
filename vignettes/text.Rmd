...
The following examples illustrate each command.

### Mark text

The `mark` command allows highlighting text by plotting it with colored background boxes (to provide the functionality of "Markieren"):

```{r mark, echo = FALSE, fig.width = 3, fig.height = 3}
lbl_blank <- c("                                                ",
              "                                      ",
              "                                                      ",
              "                                                ",
...
The text labels to be added are provided (as a character vector) by the `lbls` argument, their coordinates as the corresponding `x` and `y` arguments. 
...
If no pre-existing plot is available, setting the `new_plot` argument to either `"blank"` or `"slide"` will generate a new default plot (by using the `slide` function described above). Instead of providing explicit `x` and `y` coordinates for all text elements in `lbls`, the following examples define an initial coordinate and use a numeric `y_layout` argument to fix the distance between consecutive text labels: 
...
Two special values for `y_layout` are 
