...
```{r pal-all, fig.align = 'center', fig.width = 6, out.width = "70%", fig.asp = .90}
seecol("all_unikn")
```

The distinctive and recognizable colors of these palettes were defined in the local **Corporate Design Manual** (see the [corporate design](https://www.uni-konstanz.de/en/university/news-and-media/create-online-and-print-media/corporate-design/) specifications). 
...
```{r pal-unikn, eval = TRUE, fig.width = 7, fig.asp = .75, fig.align = 'center'}
...
Several more specialized color palettes are combine the elements of nine different color hues. 
Specifically, an\ Excel file entitled [Colours for complex graphics](https://www.uni-konstanz.de/en/university/news-and-media/create-online-and-print-media/corporate-design/) defines 9\ color gradients that are created and exported by **unikn** as named color palettes: 

```{r pal-additional, eval = FALSE}
seecol(pal = "grad_all")
```

For instance, the 9\ preferred colors (i.e., named colors of distinct color hues) are contained in palette\ `pal_unikn_pref`: 

```{r pal-unikn-pref, eval = TRUE, fig.width = 6, fig.asp = .75, fig.align = 'center'}
# See the palette of 9 preferred colors/hues: 
seecol(pal_unikn_pref)
```

All these color palettes are exported as data frames that can be accessed by number, value, or name:
...
```{r seecol-add-pals, fig.width = 7.5, out.width = "70%", fig.align = 'center', fig.asp = .90}
seecol("add", main = "Color palettes from various institutions")
```

These palettes are exported (as named vectors), documented (to credit their contributors and sources), and can be used by the R community (e.g., when creating visualizations or modified with the **unikn** [color functions](colors.html)). 
...
```{r pal-n, eval = TRUE, fig.width = 6, fig.asp = .75, fig.align = 'center'}
